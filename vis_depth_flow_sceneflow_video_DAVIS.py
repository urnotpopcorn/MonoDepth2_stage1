...
class Evaler:
    ...
    def predict_depth_eval(self, input_tensor):
        #step0: compute disp
        MIN_DEPTH = 1e-3
        MAX_DEPTH = 80
    ...
    def predict_rigid_flow_with_ins_flow(self, inputs, cam_T_cam, depth0,
        ...
        if self.opt.instance_motion:
            return flow0_pred3, flow0_pred2, flow0_pred, mask0_pred3, mask0_pred2, mask0_pred
        elif self.opt.instance_pose:
            return flow0_pred2, flow0_pred, mask0_pred2, mask0_pred
        else:
            return flow0_pred, mask0_pred
    ...
    def predict_flow_img_pair(self, inputs, outputs):
        ...
        if self.opt.instance_pose:
            ...
            if self.opt.instance_motion:
                flow0_pred3, flow0_pred2, flow0_pred, mask0_pred3, mask0_pred2, mask0_pred = self.predict_rigid_flow_with_ins_flow(
                                        inputs, cam_T_cam, depth_1,
                                        self.inv_K, self.K,
                                        rigid_flow_pred)
                outputs["flow_stage1"] = flow0_pred
                ...
                outputs["mask_stage2"] = mask0_pred2
                outputs["mask_stage3"] = mask0_pred3
                outputs["flow"] = flow0_pred3
            elif self.opt.instance_pose:
                flow0_pred2, flow0_pred, mask0_pred2, mask0_pred = self.predict_rigid_flow_with_ins_flow(
                                        inputs, cam_T_cam, depth_1,
                                        self.inv_K, self.K,
                                        rigid_flow_pred)
                outputs["flow_stage1"] = flow0_pred
                ...
                outputs["mask_stage1"] = mask0_pred 
                outputs["mask_stage2"] = mask0_pred2
                outputs["flow"] = flow0_pred2
            # else:
            #     flow0_pred, mask0_pred = self.predict_rigid_flow_with_ins_flow(
            #                             inputs, cam_T_cam, depth_1,
            #                             self.inv_K, self.K,
            #                             rigid_flow_pred)
        else:
            ...
            # outputs["flow_stage1"] = rigid_flow_pred
        
        outputs["mask"] = torch.sum(inputs["img_1_ins"][:, 1:, :, :], 1).unsqueeze(1).float()
        return outputs
    ...
    def load_img(self, input_dir, img_ext, i, crop_flag=False):
        input_path_1 = os.path.join(input_dir, str(i).zfill(5)+"."+img_ext)
        input_path_2 = os.path.join(input_dir, str(i+1).zfill(5)+"."+img_ext)
        ...
        if crop_flag == True:
            ...
            w_h_ratio_tgt = self.opt.width * 1.0 / self.opt.height
        ...
        return tensorImg1, tensorImg2, height, width
    ...
    # def load_sem(self, input_dir, img_ext, i):
    #     input_path_1 = os.path.join(input_dir, str(i).zfill(5)+"."+img_ext)
    #     input_path_2 = os.path.join(input_dir, str(i+1).zfill(5)+"."+img_ext)
    ...
    def extract_ins(self, ins_path, crop_flag):
        ...
        if crop_flag == True:
            ...
            sig_ins_id_seg = sig_ins_id_seg.crop((left, top, right, bottom))
    ...
    def extract_bbox_ins_edge(self, bbox_path, ins_path):
        ins_seg = self.get_sem_ins(ins_path)
        sig_ins_id_seg = PIL.Image.fromarray(np.uint8(ins_seg[:,:,1])).convert("L")
    ...
    def load_ins(self, ins_dir, i, crop_flag=False):
        ins_path_1 = os.path.join(ins_dir, str(i).zfill(5)+".npy")
        ins_path_2 = os.path.join(ins_dir, str(i+1).zfill(5)+".npy")
        
        ins_onehot_1 = self.extract_ins(ins_path_1, crop_flag)
        ins_onehot_2 = self.extract_ins(ins_path_2, crop_flag)

        return ins_onehot_1, ins_onehot_2

    def load_bbox_ins_edge(self, bbox_dir, ins_dir, i):
        bbox_path_1 = os.path.join(bbox_dir, str(i).zfill(5)+".txt")
        ins_path_1 = os.path.join(ins_dir, str(i).zfill(5)+".npy")
        
        bbox_path_2 = os.path.join(bbox_dir, str(i+1).zfill(5)+".txt")
        ins_path_2 = os.path.join(ins_dir, str(i+1).zfill(5)+".npy")
        
        ins_RoI_bbox_1, ins_onehot_1, ins_edge_1 = self.extract_bbox_ins_edge(bbox_path_1, ins_path_1)
        ins_RoI_bbox_2, ins_onehot_2, ins_edge_2 = self.extract_bbox_ins_edge(bbox_path_2, ins_path_2)
    ...
    def save_everything(self, outputs, output_dir, idx, inputs, seq, height=None, width=None, resize_flag=False):
        ...
        mask_dir = os.path.join(output_dir, 'mask', seq)
        ...
        output_file = str(idx).zfill(5)

        plt.figure()
        depth = np.array(outputs["depth0_vis"].squeeze().cpu())
        if resize_flag == True:
            depth_max = np.max(depth)
            depth = 255.0 * depth / depth_max
            depth = cv2.resize(depth, (width, height))
            depth = depth * depth_max / 255.0
        
        plt.imshow(1.0 / depth, cmap="plasma")
        ...
        if resize_flag == True:
            flow = fl.resize_flow(flow, width, height)
        else:
        ...
        if "mask" in outputs:
            ...
            plt.figure()
            mask = outputs["mask"].squeeze().cpu()
            plt.imshow(mask)
            output_path = os.path.join(mask_dir, output_file+"_mask.jpg")
            plt.axis('off')
            plt.savefig(output_path, bbox_inches='tight', pad_inches=0.0)
            plt.close()
    ...
    def visualize(self, outputs, output_dir, idx, inputs=None, seq=None):
        ...
        plt.subplot(6, 2, 4)
        plt.imshow(outputs["mask"].squeeze().cpu())
        plt.title("Mask")
        ...
        if self.opt.instance_motion:
            ...
            plt.subplot(6, 2, 7)
            mask = outputs["mask_stage3"].squeeze().cpu().numpy()
            plt.imshow(mask)
            plt.title("mask_stage3")

            plt.subplot(6, 2, 8)
            flow = outputs["flow_stage2"].squeeze().cpu().numpy()
            flow = fl.resize_flow(flow, self.opt.width, self.opt.height)
            flow = fl.flow_to_image(flow)
            plt.imshow(flow)
            plt.title("flow_stage2")

            plt.subplot(6, 2, 9)
            mask = outputs["mask_stage2"].squeeze().cpu().numpy()
            plt.imshow(mask)
            plt.title("mask_stage2")

            plt.subplot(6, 2, 10)
            flow = outputs["flow_stage1"].squeeze().cpu().numpy()
            flow = fl.resize_flow(flow, self.opt.width, self.opt.height)
            flow = fl.flow_to_image(flow)
            plt.imshow(flow)
            plt.title("flow_stage1")

            plt.subplot(6, 2, 11)
            mask = outputs["mask_stage1"].squeeze().cpu().numpy()
            plt.imshow(mask)
            plt.title("mask_stage1")
    ...
    def predict_depth_img_pair(self, inputs, outputs):
        ...
        outputs["depth0_vis"] = self.predict_depth_eval(inputs["tensorImg1"])
        outputs["depth1_vis"] = self.predict_depth_eval(inputs["tensorImg2"])
    ...
    def prepare_inputs(self, tensorImg1, tensorImg2, img_1_ins, img_2_ins):
        inputs = {}
        inputs["tensorImg1"] = tensorImg1
        inputs["tensorImg2"] = tensorImg2
        inputs["img_1_ins"] = img_1_ins # highlight
        inputs["img_2_ins"] = img_2_ins # highlight
...
if __name__ == '__main__':
    ...
    vis_flag = False
    save_per_image = True
    if 'DAVIS' not in opts.output_dir:
        print('OutputDir Error:\t', opts.output_dir)
        input()

    # /mnt/sdc/xzwu/Dataset/DAVIS_2017/DAVIS-2017-Unsupervised-trainval-Full-Resolution/JPEGImages/Full-Resolution/sheep/
    # input_dir = "/home/xzwu/xzwu/Code/GenerateSemantic/Dataset/DAVIS_2017/DAVIS-2017-Unsupervised-trainval-Full-Resolution/JPEGImages/Full-Resolution"
    # sem_dir = "/home/xzwu/xzwu/Code/GenerateSemantic/Dataset/DAVIS_2017/DAVIS-2017-Unsupervised-trainval-Full-Resolution/SIG/sem"
    # ins_dir = "/home/xzwu/xzwu/Code/GenerateSemantic/Dataset/DAVIS_2017/DAVIS-2017-Unsupervised-trainval-Full-Resolution/SIG/ins"
    # bbox_dir = "/home/xzwu/xzwu/Code/GenerateSemantic/Dataset/DAVIS_2017/DAVIS-2017-Unsupervised-trainval-Full-Resolution/SIG/bbox"

    # input_dir = "/home/xzwu/xzwu/Code/GenerateSemantic/Dataset/DAVIS_2017/DAVIS-2019-Unsupervised-test-dev-Full-Resolution/JPEGImages/Full-Resolution"
    ...
    input_dir = "/home/xzwu/xzwu/Code/GenerateSemantic/Dataset/DAVIS_2017/DAVIS-2019-Unsupervised-test-dev-Full-Resolution_selected/JPEGImages/Full-Resolution"
    sem_dir = "/home/xzwu/xzwu/Code/GenerateSemantic/Dataset/DAVIS_2017/DAVIS-2019-Unsupervised-test-dev-Full-Resolution_selected/SIG/sem"
    ins_dir = "/home/xzwu/xzwu/Code/GenerateSemantic/Dataset/DAVIS_2017/DAVIS-2019-Unsupervised-test-dev-Full-Resolution_selected/SIG/ins"
    bbox_dir = "/home/xzwu/xzwu/Code/GenerateSemantic/Dataset/DAVIS_2017/DAVIS-2019-Unsupervised-test-dev-Full-Resolution_selected/SIG/bbox"
    ...
    with torch.no_grad():
        for cur_class in class_list:
            ...
            for i in tqdm(range(total_img-1)):
                try:
                    # load img pair
                    tensorImg1, tensorImg2, height, width = evaler.load_img(os.path.join(input_dir, cur_class), img_ext, i, crop_flag=False)
                    '''
                    img_1_sem, img_2_sem = evaler.load_sem(os.path.join(sem_dir, cur_class), "npy", i)
                    img_1_bbox, img_1_ins, img_1_edge, img_2_bbox, img_2_ins, img_2_edge = \
                        evaler.load_bbox_ins_edge(os.path.join(bbox_dir, cur_class), 
                                                os.path.join(ins_dir, cur_class), 
                                                i)
                    inputs = evaler.prepare_inputs(tensorImg1, img_1_sem, img_1_bbox, img_1_ins, img_1_edge)
                    '''
                    img_1_ins, img_2_ins = evaler.load_ins(os.path.join(ins_dir, cur_class), i, crop_flag=False)
                    inputs = evaler.prepare_inputs(tensorImg1, tensorImg2, img_1_ins, img_2_ins)
                    ...
                    # visualization
                    if vis_flag == True:
                        evaler.visualize(outputs, output_dir, i, inputs, cur_class)

                    if save_per_image == True:
                        evaler.save_everything(outputs, output_dir, i, inputs, cur_class, height, width, resize_flag=False)
