...
class Trainer:
    def __init__(self, options):
        ...
        if self.use_pose_net:
            if self.opt.pose_model_type == "separate_resnet":
                ...
                if self.opt.pose_decoder_normal_init:
                    def weight_init(m):
                        if isinstance(m, torch.nn.Linear):
                            torch.nn.init.xavier_normal_(m.weight)
                            torch.nn.init.constant_(m.bias, 0)
                            ...
                            torch.nn.init.constant_(m.weight, 1)
                            torch.nn.init.constant_(m.bias, 0)
                    self.models["pose"].apply(weight_init)
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            if early_phase or late_phase:
                ...
                if self.opt.instance_pose:
                    ...
                    # print(outputs[("delta_y_mean", 1, 0)])
                    # print(outputs[("delta_z_mean", 1, 0)])
                    pass
                ...
                '''
                self.log("train", inputs, outputs, losses, add_image=True)
                # self.log("train", inputs, outputs, losses, add_image=False)
                '''
                self.val()
                '''
            #input()
    ...
    def compute_poses_sfm(self, img1, img2, intrinsic_matrix, ratio=0.7):
        ...
        for frame_id in self.opt.frame_ids[1:]: # [-1, 1]
            ...
            if self.opt.instance_motion and self.opt.instance_motion_v2:
                ...
                for ins_id in range(instance_K_num-1, -1, -1): 
                    ...
                    pix_coords_stage2 = self.project_3d[scale](
                        cam_points, K, torch.bmm(T_static, T_dynamic))
                    pix_coords_stage3 = self.project_3d[scale](
                    ...
                    #step8.6: use diff between t_pred and t_gt to eliminate relative static area
                    if self.opt.roi_diff_thres is not None:
                        roi_abs = torch.abs(img0_pred2 * img0_pred2_ins_mask - inputs["color", 0, scale] * img0_pred2_ins_mask)
                        # roi_abs: bs, 3, 192, 640
                        roi_sum = torch.sum(roi_abs, dim=[1, 2, 3]) # bs,
                        mask_sum = torch.sum(img0_pred2_ins_mask, dim=[1, 2, 3]) # bs,
                        roi_diff = roi_sum.float() * 1.0 / (mask_sum.float()+1e-3) # bs,
                        roi_diff = roi_diff.unsqueeze(1) # [bs, 1]
                        roi_mask = roi_diff > self.opt.roi_diff_thres # [bs, 1]

                        img0_pred3_ins_mask = img0_pred3_ins_mask.view(self.opt.batch_size, -1) # [b, 1x192x640]
                        img0_pred3_ins_mask = img0_pred3_ins_mask * roi_mask.float() # [b, 1x192x640]
                        img0_pred3_ins_mask = img0_pred3_ins_mask.view(self.opt.batch_size, 1, self.opt.height, self.opt.width)
                ...
                mask0 =  torch.sum(inputs[("ins_id_seg", 0, scale)][:, 1:, :, :], 1).unsqueeze(1).float()
                outputs[("delta_x_mean", frame_id, scale)] = torch.sum(mask0_pred_latest * non_rigid_motion_map[0]) / (torch.sum(mask0_pred_latest)+1e-3)
                outputs[("delta_y_mean", frame_id, scale)] = torch.sum(mask0_pred_latest * non_rigid_motion_map[1]) / (torch.sum(mask0_pred_latest)+1e-3)
                outputs[("delta_z_mean", frame_id, scale)] = torch.sum(mask0_pred_latest * non_rigid_motion_map[2]) / (torch.sum(mask0_pred_latest)+1e-3)
        ...
        if add_image == True:
            for j in range(min(1, self.opt.batch_size)):  # write a maxmimum of four images
                ...
                for s in [0]:
                    ...
                    writer.add_image(
                        "normal_depth_{}/{}".format(s, j),
                        normalize_image(outputs[("depth", 0, s)][j]), self.step)
                    
                    writer.add_image(
                        "normal_disp_{}/{}".format(s, j),
                        normalize_image(outputs[("disp", s)][j]), self.step)

                    writer.add_image(
                        "depth_{}/{}".format(s, j),
                        outputs[("depth", 0, s)][j], self.step)
