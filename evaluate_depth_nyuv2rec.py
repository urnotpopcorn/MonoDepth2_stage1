...
def load_tensor_image(filename, args):
    ...
    if (h != args.height or w != args.width):
        img = imresize(img, (args.height, args.width)).astype(np.float32)
    img = np.transpose(img, (2, 0, 1))
    tensor_img = ((torch.from_numpy(img).unsqueeze(0)/255-0.45)/0.225).cuda()
    return tensor_img
...
def evaluate(opt):
    ...
    if opt.dataset == 'nyuv2rec':
        ...
        test_files = sorted(Path(os.path.join(opt.data_path, 'color')).files('*.png'))
        # print('{} files to test'.format(len(test_files)))
        pred_disps = []
        with torch.no_grad():
            for j in range(len(test_files)):
                # input_color = torch.pil_loader(test_files[j]).cuda()
                # input_color = load_tensor_image(test_files[j], opt) # [0, 2.44]
                
                resize_func = transforms.Resize((opt.height, opt.width),
                                            interpolation=PIL.Image.ANTIALIAS)
                input_color = resize_func(PIL.Image.open(test_files[j]).convert('RGB'))
                input_color = transforms.ToTensor()(input_color).unsqueeze(0).cuda() # [0, 1]
    ...
    if opt.dataset == 'nyuv2rec':
        gt_path = os.path.join(opt.data_path, "depth.npy")
    ...
    for i in range(pred_disps.shape[0]):
        ...
        if opt.eval_split == "eigen":
            ...
            # mask = gt_depth > 0
        elif opt.eval_split == "nyuv2rec":
            mask = np.logical_and(gt_depth > MIN_DEPTH, gt_depth < MAX_DEPTH)
            # mask = gt_depth > 0 # key difference
        else:
            mask = gt_depth > 0
