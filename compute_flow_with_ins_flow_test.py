...
class Evaler:
    ...
    def predict_poses(self, tensorImg1, tensorImg2):
        ...
        return cam_T_cam, axisangle, translation
    ...
    def predict_rigid_flow_with_ins_flow(self, inputs, cam_T_cam, depth0,
        ...
        for ins_id in range(instance_K_num-1, -1, -1): # benefit to large instance            # step4: use T_static to transform mask of each ins
            ...
            # ins_cam_T_cam: b, 4, 4
            rotation_total = axisangle[:, 0] + ins_axisangle[:, 0]
            translation_total = translation[:, 0] + ins_translation[:, 0]
            T_total2 = transformation_from_parameters(rotation_total, translation_total, invert=False)

            # delta_T = ins_cam_T_cam[:, 2, 3].clone()
            # T_dynamic = torch.eye(4).unsqueeze(0).cuda()
            # T_dynamic[:, 2, 3] = delta_T.clone()

            T_dynamic = ins_cam_T_cam.clone()
            T_total = torch.matmul(T_static, T_dynamic) # [bs, 4, 4]
            ...
            img0_pred_ins_mask_new = F.grid_sample(img1_ins_mask, ins_pix_coords, padding_mode="border")
        ...
        img0_pred2 = mask0_pred_final * img0_pred_final + (1-mask0_pred_final) * img0_pred
        flow0_pred2 = mask0_pred_final.permute(0, 2, 3, 1) * flow_pred_final + (1-mask0_pred_final.permute(0, 2, 3, 1)) * flow_pred     
        # save for vis
        img0_pred2_visual = mask0 * img0_pred_final + (1-mask0) * img0_pred
        flow0_pred2_visual = mask0.permute(0, 2, 3, 1) * flow_pred_final + (1-mask0.permute(0, 2, 3, 1)) * flow_pred
        
        outputs["0_tgt_{t}"] = inputs["tensorImg1"]
        outputs["1_src_{t+1}"] = inputs["tensorImg2"]
        outputs["2_pred_by_mono2"] = img0_pred
        outputs["3_pred_by_(mono2+ins)"] = img0_pred2_visual
        outputs["4_flow_by_mono2"] = flow_pred
        outputs["5_flow_by_(mono2+ins)"] = flow0_pred2_visual
        ...
        outputs["8_diff_mono2_(mono2+ins)"] = img0_pred2_visual - img0_pred
    ...
    def predict_flow_img_pair(self, inputs):
        ...
        cam_T_cam, axisangle, translation = self.predict_poses(inputs["tensorImg1"], inputs["tensorImg2"])
...
if __name__ == '__main__':
    ...
    selected_idx = [20, 43, 44, 45, 46, 49, 58, 91, 95, 197]
    with torch.no_grad():
        ...
        for i in selected_idx:
            ...
            tensorImg1, tensorImg2 = evaler.load_img(input_dir, input_file)
            ...
            # visualization
            if vis_flag == True:
                evaler.visualize_flow(flow_outputs, output_dir, input_file)
