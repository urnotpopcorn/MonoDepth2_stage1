...
class Evaler:
    ...
    def load_img(self, input_dir, img_ext, i):
    ...
    def visualize_flow(self, flow_outputs, output_dir, ins_dir, inputs, img_ext, i):
        input_file  = str(i).zfill(5)+"."+img_ext
        
        # output_dir: output/flow_DAVIS/${MODEL}/weights_${EPOCH}
        vis_output_dir = output_dir.replace('flow', 'vis')
        if os.path.exists(vis_output_dir) == False:
            os.makedirs(vis_output_dir)

        output_path = os.path.join(vis_output_dir, input_file)
    ...
    def prepare_inputs(self, tensorImg1, tensorImg2, 
    ...
    with torch.no_grad():
        for cur_class in class_list:
            ...
            for i in tqdm(range(total_img-1)):
                try:
                    # load img pair
                    tensorImg1, tensorImg2 = evaler.load_img(os.path.join(input_dir, cur_class), img_ext, i)
                    ...
                    inputs = evaler.prepare_inputs(tensorImg1, tensorImg2, 
                                img_1_sem, img_2_sem, img_1_bbox, img_1_ins, 
                                img_1_edge, img_2_bbox, img_2_ins, img_2_edge)
                    ...
                    # visualization
                    if vis_flag == True:
                        evaler.visualize_flow(flow_outputs, os.path.join(output_dir, cur_class), ins_dir, inputs, img_ext, i)
