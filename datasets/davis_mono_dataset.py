...
class DavisMonoDataset(data.Dataset):
    ...
    def __init__(self,
                 ...
                 opt=None,
                 mode=None):
        super(DavisMonoDataset, self).__init__()
        ...
        if self.mode == "train":
            self.data_path = os.path.join(data_path, "DAVIS-2017-Unsupervised-trainval-Full-Resolution")
        elif self.mode == "test":
            self.data_path = os.path.join(data_path, "DAVIS-2019-Unsupervised-test-dev-Full-Resolution")
        ...
        if self.opt.SIG or self.opt.instance_pose :
            # self.data_sem_path = os.path.join(self.opt.project_dir, "dataset", "davis_data_sem", self.mode)
            # self.data_ins_path = os.path.join(self.opt.project_dir, "dataset", "davis_data_ins", self.mode)
            # self.data_bbox_path = os.path.join(self.opt.project_dir, "dataset", "davis_data_bbox", self.mode)
            self.data_sem_path = os.path.join(self.data_path, "SIG", "sem")
            self.data_ins_path = os.path.join(self.data_path, "SIG", "ins")
            self.data_bbox_path = os.path.join(self.data_path, "SIG", "bbox")
        ...
        self.load_depth = False #self.check_davis_depth()
    ...
    def __getitem__(self, index):
        ...
        curclass = self.filenames[index].split()[0]
        frame_index = int(self.filenames[index].split()[1])
        ...
        for i in self.frame_idxs: #[0, -1, 1]
            ...
            i_frame_name = str(frame_index+i).zfill(5) + self.img_ext
            i_frame_path = os.path.join(self.data_path, "JPEGImages", "Full-Resolution", curclass, i_frame_name)
            if os.path.isfile(i_frame_path):
                inputs[("color", i, -1)] = self.get_davis_color(i_frame_path, do_flip)
            else:
                print("missing: ", i_frame_path)
                i_frame_name = str(frame_index).zfill(5) + self.img_ext
                i_frame_path = os.path.join(self.data_path, "JPEGImages", "Full-Resolution", curclass, i_frame_name)
                inputs[("color", i, -1)] = self.get_davis_color(i_frame_path, do_flip)
        ...
        if self.opt.SIG or self.opt.instance_pose:
            ...
            ins_height = dict()
            for i in self.frame_idxs: # [0,1,-1]
                i_frame_name = str(frame_index+i).zfill(5) + ".npy"
                #i_frame_path = os.path.join(self.data_ins_path, i_frame_name)
                i_frame_path = os.path.join(self.data_path, "SIG", "ins", curclass, i_frame_name)
                if os.path.isfile(i_frame_path):
                    ins_seg = self.get_sem_ins(i_frame_path, do_flip)
                else:
                    print("missing: ", i_frame_path)
                    i_frame_name = str(frame_index).zfill(5) + ".npy"
                    #i_frame_path = os.path.join(self.data_ins_path, i_frame_name)
                    i_frame_path = os.path.join(self.data_path, "SIG", "ins", curclass, i_frame_name)
                    ins_seg = self.get_sem_ins(i_frame_path, do_flip)
            ...
            if self.opt.SIG:
                for i in self.frame_idxs: # [0,1,-1]
                    ...
                    i_frame_path = os.path.join(self.data_path, "SIG", "sem", curclass, i_frame_name)
                    if os.path.isfile(i_frame_path):
        ...
        # if self.load_depth:
        #     depth_gt = self.get_davis_depth(self.filenames[index], do_flip)
        #     inputs["depth_gt"] = np.expand_dims(depth_gt, 0)
        #     inputs["depth_gt"] = torch.from_numpy(inputs["depth_gt"].astype(np.float32))

        return inputs

    def get_davis_color(self, frame_index,  do_flip):
        raise NotImplementedError

    def check_davis_depth(self):
        raise NotImplementedError

    def get_davis_depth(self, frame_index,  do_flip):
        raise NotImplementedError
