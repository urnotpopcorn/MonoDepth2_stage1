...
class NYUDataset(data.Dataset):
    ...
    def __init__(self,
                 ...
                 num_scales,
                 is_train=False,
                 img_ext=None, opt=None, mode=None,
                 segment_path = '',
                 return_segment=False,
                 shared_dict=None,
                 use_scdepth_setting=False):
        ...
        self.use_scdepth_setting = use_scdepth_setting
    ...
    def get_color(self, fp, do_flip):
        if fp in self.img_cache:
            ...
            if not(self.debug) and not(self.use_scdepth_setting):
...
class NYUTestDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        K[0, :] *= self.width
        K[1, :] *= self.height
        return rgb, depth, norm, norm_mask, K, np.linalg.pinv(K), line
