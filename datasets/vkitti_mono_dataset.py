...
class VKittiMonoDataset(data.Dataset):
    ...
    def __init__(self,
                 ...
                 opt=None,
                 mode=None):
        super(VKittiMonoDataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        # Scene06 30-deg-right    rgb_00048.jpg
        curscene, curcase, curfilename = self.filenames[index].split()
        frame_index = int(curfilename.split('.')[0].split('_')[1])

        # ~/xzwu/Dataset/VKitti/Scene01/15-deg-left/frames/rgb/Camera_0
        for i in self.frame_idxs: #[0, -1, 1]
            i_frame_name = "rgb_"+str(frame_index+i).zfill(5) + self.img_ext
            i_frame_path = os.path.join(self.data_path, curscene, curcase, "frames", "rgb", "Camera_0", i_frame_name)
            if os.path.isfile(i_frame_path):
                inputs[("color", i, -1)] = self.get_vkitti_color(i_frame_path, do_flip)
            else:
                print("missing: ", i_frame_path)
                i_frame_name = "rgb_"+str(frame_index).zfill(5) + self.img_ext
                i_frame_path = os.path.join(self.data_path, curscene, curcase, "frames", "rgb", "Camera_0", i_frame_name)
                inputs[("color", i, -1)] = self.get_vkitti_color(i_frame_path, do_flip)
    ...
    def get_vkitti_color(self, frame_index,  do_flip):
