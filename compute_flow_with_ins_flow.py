...
class Evaler:
    ...
    def add_non_rigid_motion(self, points, non_rigid_motion_map,
        ...
        cam_points = points[:, :3, :].clone() # bs, 3, 122880
    ...
    def compute_ins_pose_flow(self, cam_points0_pred, T_static, ins_pix_coords, scale=0):
        # FIXME: 
        # flow_ins_pix_coords = self.project_3d[scale](cam_points, K, T_total)
        # ins_pose_flow_pred = self.extract_flow(ins_pix_coords)
        ins_base_pix_coords = self.project_3d[scale](cam_points0_pred, self.K[scale], T_static)
    ...
    def predict_rigid_flow_with_ins_flow(self, inputs, cam_T_cam, depth0,
        ...
        # warp semantic image
        if self.opt.SIG:
            img0_pred_sem = F.grid_sample(inputs["img_2_sem"], pix_coords, padding_mode="border")
            img0_pred_edge = F.grid_sample(inputs["img_2_edge"], pix_coords, padding_mode="border")
        ...
        for ins_id in range(instance_K_num-1, -1, -1): # benefit to large instance            # step4: use T_static to transform mask of each ins
            ...
            cam_points0_pred = self.backproject_depth[scale](
                img_z, inv_K) # cam_points of frame 0, [12, 4, 122880]
        ...
        if self.opt.instance_motion and self.opt.instance_motion_v2:
            ...
            if self.opt.eval_flow_filter_warping_error_stage3:
                ...
                # flow0_pred3 = flow_mask.permute(0, 2, 3, 1) * flow0_pred3 + (1-flow_mask).permute(0, 2, 3, 1) * flow0_pred2
        ...
        outputs["6_mask_pred_by_mono2"] = self.filter_mask(F.grid_sample(torch.sum(inputs["img_2_ins"][:, 1:, :, :], 1).unsqueeze(1).float(), pix_coords, padding_mode="border"))
...
if __name__ == '__main__':
    ...
    selected_idx = [18,19,20, 43, 44, 45, 46, 49, 58, 91, 95, 184, 193, 197]
    # selected_idx = [20, 95]
    with torch.no_grad():
