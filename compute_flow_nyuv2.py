...
class Evaler:
    ...
    def load_pose(self, input_dir, idx):
        # input_dir = dataset/NYUv2/NYUv2_labeled/processed_data/pose
        input_path_1 = os.path.join(input_dir, "pose.npy")
    ...
    def predict_flow_img_pair(self, inputs):
        ...
        cam_T_cam = inputs["tensorPose"]
        flow_outputs = self.predict_rigid_flow(cam_T_cam, depth_1, self.inv_K, self.K, inputs["tensorImg2"])

        return flow_outputs

    def load_img(self, input_dir, img_ext='jpg', idx=None):
        input_path_1 = os.path.join(input_dir, str(idx).zfill(5)+"."+img_ext)
        input_path_2 = os.path.join(input_dir, str(idx+1).zfill(5)+"."+img_ext)
...
if __name__ == '__main__':
    ...
    input_dir = "dataset/NYUv2/NYUv2_labeled/processed_data/"
    img_ext = "jpg"
    total_img = 1449
    ...
    with torch.no_grad():
        ...
        # for i in selected_idx:
            # load img pair
            inputs = {}
            tensorImg1, tensorImg2 = evaler.load_img(os.path.join(input_dir, 'rgb'), img_ext, idx=i)
            tensorPose = evaler.load_pose(os.path.join(input_dir, 'pose'), i)
            
            inputs["tensorImg1"] = tensorImg1
            inputs["tensorImg2"] = tensorImg2
            inputs["tensorPose"] = tensorPose
