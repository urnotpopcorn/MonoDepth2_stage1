    ...
    with torch.no_grad():
        for cur_class in class_list:
            ...
            for cur_type in type_list: # type_list
                ...
                sub_sub_dir = os.path.join(sub_dir, cur_type, 'frames', 'rgb', 'Camera_0')
                ...
                for i in tqdm(range(total_img-1)):
                    try:
                        # load img pair
                        input_file = "rgb_"+str(i).zfill(5)+"."+img_ext
                        tensorImg1 = evaler.load_img(sub_sub_dir, input_file)
                        inputs = evaler.prepare_inputs(tensorImg1)
                        disp1 = evaler.predict_disp_img_pair(inputs)

                        # save flow
                        if save_flag == True:
                            output_path = os.path.join(output_dir, cur_class, cur_type, 'Camera_0')
