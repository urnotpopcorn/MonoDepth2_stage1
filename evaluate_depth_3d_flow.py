...
class Evaler:
    ...
    def predict_depth_new(self, input_tensor):
        #step0: compute disp
        features = self.models["encoder"](input_tensor)
        outputs = self.models["depth"](features)
        disp = outputs[("disp", 0)]
        ...
        depth_eval = 1.0 / disp
        depth_eval[depth_eval > MAX_DEPTH] = MAX_DEPTH
        depth_eval[depth_eval < MIN_DEPTH] = MIN_DEPTH

        return depth, depth_eval
    ...
    def predict_3d_flow_tgt(self, cam_T_cam, depth_1, depth_2, 
                            optical_flow, inv_K_dict, K_dict, 
                            img1=None):
        source_scale = 0
        inv_K = inv_K_dict[source_scale]
        K = K_dict[source_scale]
        ...
        cam_points_2_after = self.transform_3d[source_scale](cam_points_2_after, inv_T) # tgt
    ...
    def predict_3d_flow_zero(self, cam_T_cam, depth_1, depth_2, optical_flow, inv_K_dict, K_dict, img1=None):
    ...
    def predict_3d_flow_with_ins_flow(self, inputs, cam_T_cam, depth0, depth_1,
        ...
        if self.opt.instance_motion:
            outputs["flow_stage3"] = sceneflow0_pred3.clone()
            ones = nn.Parameter(torch.ones(self.opt.batch_size, 1, self.opt.height * self.opt.width),
                                 requires_grad=False).cuda()
            sceneflow0_pred3 = torch.cat([sceneflow0_pred3.view(self.opt.batch_size, 3, -1), ones], 1) # bs, 4, 12288
            cam_points_src = self.transform_3d[scale](cam_points + sceneflow0_pred3, T_static)
            cam_points_src = cam_points_src.view(self.opt.batch_size, -1, self.opt.height, self.opt.width)
            depth_1_new = cam_points_src[:, 2:3, :, :]
            outputs["depth_2_new"] = depth_1_new
        elif self.opt.instance_pose:
            outputs["flow_stage2"] = sceneflow0_pred2.clone()
            ones = nn.Parameter(torch.ones(self.opt.batch_size, 1, self.opt.height * self.opt.width),
                                 requires_grad=False).cuda()
            sceneflow0_pred2 = torch.cat([sceneflow0_pred2.view(self.opt.batch_size, 3, -1), ones], 1) # bs, 4, 12288
            cam_points_src = self.transform_3d[scale](cam_points + sceneflow0_pred2, T_static)
            cam_points_src = cam_points_src.view(self.opt.batch_size, -1, self.opt.height, self.opt.width)
            depth_1_new = cam_points_src[:, 2:3, :, :]
            outputs["depth_2_new"] = depth_1_new
        else:
    ...
    def predict_depth_img_pair(self, inputs, outputs, depth_2, mask=None):
        if self.opt.instance_motion or self.opt.instance_pose:
            depth_2_new = outputs["depth_2_new"]
            if mask is None:
                depth_2_final = (depth_2 + depth_2_new) / 2.0
                return depth_2_final
            else:
                depth_2_final = mask * depth_2_new + (1-mask) * depth_2
                return depth_2_final
        else:
            return depth_2
    ...
    def predict_flow_img_pair(self, inputs):
        if self.opt.SIG:
            ...
            depth_1, depth_1_eval = self.predict_depth_new(inputs["tensorImg1"])
            depth_2, depth_2_eval = self.predict_depth_new(inputs["tensorImg2"])
        ...
        if self.opt.instance_pose:
            ...
            if self.opt.SIG:
                pass
                # flow_outputs = self.predict_3d_flow_with_ins_flow(
            ...
            optical_flow_outputs = self.predict_rigid_flow(cam_T_cam, depth_1, self.inv_K, self.K, inputs["tensorImg2"])
            flow_outputs = self.predict_3d_flow_tgt(cam_T_cam, depth_1, depth_2, optical_flow_outputs["flow_stage1"],
                                                self.inv_K, self.K, inputs["tensorImg2"])

        return flow_outputs, depth_1_eval, depth_2_eval

    def load_img(self, input_dir, input_file, img_ext='png', idx=None):
        input_path_1 = os.path.join(input_dir, input_file+"_11."+img_ext)
        input_path_2 = os.path.join(input_dir, input_file+"_10."+img_ext)
    ...
    def load_sem(self, input_dir, input_file):
        input_path_1 = os.path.join(input_dir, input_file+"_11.npy")
        input_path_2 = os.path.join(input_dir, input_file+"_10.npy")
    ...
    def load_bbox_ins_edge(self, bbox_dir, ins_dir, input_file):
        ins_RoI_bbox_1, ins_onehot_1, ins_edge_1 = self.extract_bbox_ins_edge(input_file+"_11.npy")
        ins_RoI_bbox_2, ins_onehot_2, ins_edge_2 = self.extract_bbox_ins_edge(input_file+"_10.npy")
...
def compute_depth_errors(gt_depth, pred_depth):
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for i in tqdm(range(total_img)):
            ...
            inputs = {}
            tensorImg1, tensorImg2 = evaler.load_img(input_dir, input_file, img_ext, idx=i)
            ...
            flow_outputs, depth_1_eval, depth_2_eval = evaler.predict_flow_img_pair(inputs)
            # t4 = timeit.default_timer()

            mask_2 = torch.sum(inputs["img_2_ins"][:, 1:, :, :], 1).squeeze().float()
            depth_2_final = evaler.predict_depth_img_pair(inputs, flow_outputs, depth_2_eval, mask_2)
            depth_2_final = depth_2_final.squeeze().cpu().numpy()
            errors.append(compute_depth_errors(gt_depths[i], depth_2_final))
