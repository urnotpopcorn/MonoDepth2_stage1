...
class Evaler:
    ...
    def predict_rigid_flow_with_ins_flow(self, inputs, cam_T_cam, depth0,
        ...
        # flow0_pred2 = mask0_pred2_base.permute(0, 2, 3, 1) * flow0_pred2_base + (1-mask0_pred2_base.permute(0, 2, 3, 1)) * flow0_pred
        flow0_pred2 = mask0.permute(0, 2, 3, 1) * flow0_pred2_base + (1-mask0.permute(0, 2, 3, 1)) * flow0_pred
        ...
        if self.opt.instance_motion and self.opt.instance_motion_v2:
            ...
            flow0_pred3 = mask0.permute(0, 2, 3, 1) * flow0_pred3_base + (1-mask0.permute(0, 2, 3, 1)) * flow0_pred2
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        selected_idx = [2, 18,19,20, 43, 44, 45, 46, 49, 58, 91, 95, 100, 184, 193, 197]
        # selected_idx = [2, 3, 100, 168, 169, 188, 189]
        # selected_idx = [0, 7, 18, 19, 25, 69, 106, 111, 117, 149]
        # selected_idx = [72]
        # selected_idx = [128, 48, 105, 33, 184, 29, 41, 106, 45, 165, 25, 190, 117, 26, 31, 177] # kitti raw data
        for i in tqdm(range(total_img)):
        # for i in selected_idx:
            if opts.dataset == 'kitti_raw_data_flow' and (i in invalid_next_idx or i in invalid_pre_idx):
                continue 
