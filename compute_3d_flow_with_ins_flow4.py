...
class Evaler:
    ...
    def predict_rigid_flow_with_ins_flow(self, inputs, cam_T_cam, depth0,
        ...
        for ins_id in range(instance_K_num-1, -1, -1): # benefit to large instance            # step4: use T_static to transform mask of each ins
            ...
            # if self.opt.use_depth_ordering:
            ins_pix_coords, img_z_stage2 = self.project_3d[scale](cam_points, K, T_dynamic, return_z=True)
        ...
        if self.opt.instance_motion and self.opt.instance_motion_v2:
            ...
            pix_coords_stage3, img_z_stage3 = self.project_3d[scale](
                cam_points0_pred_stage3, K, torch.eye(4).to(self.device), return_z=True)
            ...
            if self.opt.predict_img_motion:
                img0_pred3 = img0_pred3_base.clone()
            ...
            if self.opt.eval_flow_filter_warping_error_stage3:
                ...
                # flow0_pred3_visual = flow_mask.permute(0, 2, 3, 1) * flow0_pred3_visual + (1-flow_mask).permute(0, 2, 3, 1) * flow0_pred2_visual
                flow0_pred3 = flow_mask * flow0_pred3 + (1-flow_mask) * flow0_pred2
                flow0_pred3_visual = flow_mask * flow0_pred3_visual + (1-flow_mask) * flow0_pred2_visual
    ...
    def predict_3d_flow_with_ins_flow(self, inputs, cam_T_cam, depth0, depth_1,
        ...
        if self.opt.instance_motion:
            outputs["flow_stage3"] = sceneflow0_pred3
        elif self.opt.instance_pose:
            outputs["flow_stage2"] = sceneflow0_pred2
        else:
            pass
